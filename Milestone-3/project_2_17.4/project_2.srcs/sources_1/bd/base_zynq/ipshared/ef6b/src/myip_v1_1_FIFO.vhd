...
entity myip_v1_1_FIFO is
	...
	port (
		...
		FIFO_2_LAST         : out std_logic;
        LAST_2_FIFO         : in std_logic
	);
end myip_v1_1_FIFO;
...
architecture Behavioral of myip_v1_1_FIFO is
	...
	-- Implement FIFO as an array of logic_vectors
    type FIFO_LAST_TYPE is array (0 to (FIFO_DEPTH)) of std_logic;
    signal FIFO_LAST : FIFO_LAST_TYPE := (others => '0');
...
begin
	...
	FIFO_2_LAST   <= FIFO_LAST(read_pointer);
			...
			if (FIFO_ARESETN = '0') then
				...
				if (FIFO_WEN = '1') THEN
					...
					if (full = '0') then
						...
						FIFO_LAST(write_pointer) <= LAST_2_FIFO;
